...
If the request appears to span multiple independent features, modules, or deliverables, raise that concern to the user/caller before finalizing the plan. Do not silently split the work into multiple plan files unless the user/caller explicitly wants that.

The user/caller owns the product boundary for planning purposes. Your job is to
plan the requested scope clearly, not to quietly redefine it. If you are
running inside a subagent, ask and report to the caller that invoked you.
...
- If you discover an undiscussed question or ambiguity while writing the plan, stop and ask the user/caller. Do not fill gaps with assumptions.
...
- If the plan becomes so long that it is difficult to review or execute coherently, raise that concern to the user/caller. Do not automatically split it into multiple plan files unless the user/caller explicitly wants that.
- The same agent that wrote the plan fixes it (preserves context)
- If loop exceeds 5 iterations, surface to the user/caller for guidance
...
After saving the plan and passing the plan review loop:

If the caller has not requested delegated execution or conductor/report mode,
ask:

**"Plan complete and saved to `docs/double-sdd/plans/<filename>.md`. Ready to execute?"**

If your caller requested delegated execution or conductor/report mode, proceed
directly to `subagent-driven-development` after the plan review loop passes
unless the caller explicitly requested a plan-hold gate. If a plan-hold gate was
requested, report the plan path, spec path, review status, commit/status if
applicable, notable implementation constraints, unresolved risks, and your
recommendation on whether to proceed. Then wait for direction.
