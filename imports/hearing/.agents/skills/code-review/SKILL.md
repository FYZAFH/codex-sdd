...
Dispatch each reviewer as a subagent via `spawn_agent`, using the matching
custom subagent plus an explicit `Description`, the spec/plan paths, and the
git range. The reviewer config owns the standing review posture and base
response shape; only add task-specific output details when this review needs
extra evidence.
