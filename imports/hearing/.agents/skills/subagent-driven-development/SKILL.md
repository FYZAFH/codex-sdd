...
**IMPORTANT:** Do NOT assume these artifacts exist based on the topic being discussed or files you find by searching. A spec/plan only counts as "existing" if the user/caller has **explicitly pointed you to it** (e.g., by providing the path directly). If the user/caller has not explicitly indicated these files, treat them as missing and invoke the corresponding skill.
...
- for named custom subagents, keep the message as a task packet; do not restate
  role, workflow, or standing instructions already owned by the agent config
...
- supplement the agent's normal report structure only when this task needs
  extra evidence or a task-specific section
...
3. If the plan itself is wrong, verify the blocker is real. Then:
      ...
      - Otherwise → escalate to the user/caller
...
If running in conductor/report mode, invoke `finishing-a-development-branch`
in report mode. Do not present the merge/PR/discard option menu unless your
caller explicitly asked for direct option mode.
