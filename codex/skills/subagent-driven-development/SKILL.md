...
Before executing any tasks, create an isolated worktree from a recorded main-worktree base. Use the non-committed JSON metadata file created by `writing-specs`, or create one only through direct/manual no-upstream mode with `--no-upstream-metadata --run-id <filesystem-safe-run-id>`.
...
- Run setup from the original main worktree only. If the current checkout is already a linked worktree, return to the original main worktree before deriving paths or dispatching agents.
- Derive `.worktrees`, metadata paths, and worktree paths from the resolved repository root. `.worktrees` must be repo-local, git-ignored, and rejected if it is a symlink, Windows junction, mount point, or other reparse point.
- Confirm both `.worktrees/` and the exact metadata path are ignored by git before writing runtime metadata. The exact metadata path contract is `.worktrees/<run-id>.metadata.json`: one top-level filename under `.worktrees`, repo-relative POSIX style, no absolute paths, no parent traversal, and no nested metadata paths.
- Use upstream metadata-path mode when `writing-specs` hands off a metadata file. Use automatic upstream candidate lookup only when no metadata path and no direct/manual flags are supplied. Use direct/manual no-upstream mode only with `--no-upstream-metadata --run-id <filesystem-safe-run-id>`.
- Direct/manual no-upstream creation requires a clean main worktree with no preexisting tracked, untracked, or unsafe ignored state that this workflow cannot record. If blocking state exists, stop and require the upstream recording step instead of writing empty `preexisting*` defaults.
- Reused metadata must be active, complete, well-typed, and pre-setup: `featureBranch` and `worktreePath` are unset/null or empty, `mainBranch` is non-empty, `metadataPath` matches the exact file being read, and `runId` is filesystem-safe using only ASCII letters, numbers, `.`, `_`, and `-` with no `..`.
- Validate `mainBase` as a 40-character commit SHA and verify it resolves to a commit before creating the worktree. Use the recorded `mainBase` for reused metadata and current `HEAD` only for direct/manual metadata creation.
- Keep `worktreePath` inside the verified repo-local `.worktrees` directory for this workflow. Persist `metadataPath` and `worktreePath` as repo-relative POSIX-style paths; use absolute paths only for filesystem operations.
- Update `featureBranch` and `worktreePath` only after the worktree is successfully added. Immediately before the update, re-read and revalidate metadata, then confirm `runId`, `status`, `metadataPath`, `featureBranch`, and `worktreePath` still match the expected lifecycle state.
- If the target branch or worktree path already exists while metadata is incomplete, fail closed unless the existing branch, worktree path, `mainBase`, and `runId` can be verified as the intended state. Otherwise stop with recovery guidance such as choosing a new branch/path, removing stale state after user approval, or manually repairing metadata.
- If metadata update fails after worktree creation, do not dispatch agents or claim setup success. Stop with rollback/reconcile guidance: inspect the created worktree/branch, reconcile or replace the metadata file, or remove the worktree/branch only after verifying they contain no wanted changes.
- Existing metadata reads must normalize `OSError`, `UnicodeDecodeError`, and `json.JSONDecodeError` into clear reconcile-or-replace guidance rather than raw tracebacks.
- Metadata starts with `status: "active"`; finishing later marks it `completed` or `abandoned`. Do not store file contents, environment variables, tokens, or command output in the metadata artifact.

Run project setup (npm install / cargo build / pip install / go mod download) and verify tests pass before proceeding.

When dispatching downstream agents, pass the printed `worktreePath` as the working directory and the metadata path when relevant so fresh implementers, reviewers, and the finishing workflow can read recorded state instead of relying on hidden session memory.
...
Dispatch with: description, spec/plan paths, working directory from the printed `worktreePath`, metadata path, git range (Base..Head).
...
- include the printed `worktreePath` as the working directory (`Work from`) for implementers and reviewers
